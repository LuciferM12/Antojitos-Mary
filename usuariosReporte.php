...
class PDF extends FPDF
{
   ...
   function Header()
   {
      ...
      $this->Cell(55, 10, utf8_decode('telefono'), 1, 1, 'C', 1);
   }
...
$pdf = new PDF();
$pdf->AddPage("L"); /* aqui entran dos para parametros (horientazion,tamaño)V->portrait H->landscape tamaño (A3.A4.A5.letter.legal) */
...
$compra = $conn->query("select idUsuario, rol, correo, nombre, telefono From Usuario");

while($row = $compra->fetch_object()){
    $pdf->Cell(18, 10, utf8_decode($row->idUsuario), 1, 0, 'C', 0);
    $pdf->Cell(30, 10, utf8_decode($row->rol), 1, 0, 'C', 0);
    $pdf->Cell(80, 10, utf8_decode($row->correo), 1, 0, 'C', 0);
    $pdf->Cell(85, 10, utf8_decode($row->nombre), 1, 0, 'C', 0);
    $pdf->Cell(55, 10, utf8_decode($row->telefono), 1, 1, 'C', 0);
  ...
  $pdf->Cell(85, 10, utf8_decode("El presente listado presenta los reportes de todos los usuarios registrados al momento"), 0, 0, '', 0);
  $pdf->Ln(5);
