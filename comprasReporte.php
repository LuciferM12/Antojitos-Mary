...
class PDF extends FPDF
{
   ...
   function Header()
   {
      ...
      $this->Cell(80, 10, utf8_decode('Usuario'), 1, 0, 'C', 1);
      $this->Cell(80, 10, utf8_decode('Vendedor'), 1, 0, 'C', 1);
      $this->Cell(25, 10, utf8_decode('Fecha'), 1, 0, 'C', 1);
      $this->Cell(30, 10, utf8_decode('Total de venta'), 1, 1, 'C', 1);
   }
...
$compra = $conn->query("select * From Compra");

while($row = $compra->fetch_assoc()){
    $consulta = "Select nombre From Usuario where idUsuario =". $row["idUsuario"];
    $usuario = $conn->query($consulta);
    $usuario2 = $usuario->fetch_assoc();
    
    $vendedor = $conn->query("Select nombre From Usuario Where idUsuario=".$row["idVendedor"]);
    $vendedor2 = $vendedor->fetch_assoc();
    
    $pdf->Cell(18, 10, utf8_decode($row["idCompra"]), 1, 0, 'C', 0);
    $pdf->Cell(80, 10, utf8_decode($usuario2["nombre"]), 1, 0, 'C', 0);
    $pdf->Cell(80, 10, utf8_decode($vendedor2["nombre"]), 1, 0, 'C', 0);
    $pdf->Cell(25, 10, utf8_decode($row["fecha"]), 1, 0, 'C', 0);
    $pdf->Cell(30, 10, utf8_decode($row["costoTotal"]), 1, 1, 'C', 0);
}
$compra ->free();
  $pdf->Cell(1);  // mover a la derecha
  $pdf->SetFont('Arial', 'B', 10);
  $pdf->Cell(85, 10, utf8_decode("El presente listado presenta los reportes de todas las compras realizadas al día generado"), 0, 0, '', 0);
  $pdf->Ln(5);
